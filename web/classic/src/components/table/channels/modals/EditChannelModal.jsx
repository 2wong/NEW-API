...
const STATUS_CODE_RETRY_DEFAULT_STATUS_CODES = '429';
const STATUS_CODE_RETRY_DEFAULT_COUNT = 1;
const STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID = 0;
const STATUS_CODE_RETRY_DEFAULT_INTERVAL_SECONDS = 0;
...
const EditChannelModal = (props) => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.status_code_retry_enabled =
            parsedSettings.status_code_retry_enabled === true;
          data.status_code_retry_status_codes =
            parsedSettings.status_code_retry_status_codes ||
            STATUS_CODE_RETRY_DEFAULT_STATUS_CODES;
          data.status_code_retry_count = parsePositiveInteger(
            parsedSettings.status_code_retry_count,
            STATUS_CODE_RETRY_DEFAULT_COUNT,
          );
          data.status_code_retry_channel_id = parseNonNegativeInteger(
            parsedSettings.status_code_retry_channel_id,
            STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID,
          );
          data.status_code_retry_interval_seconds = parseNonNegativeInteger(
            parsedSettings.status_code_retry_interval_seconds,
            STATUS_CODE_RETRY_DEFAULT_INTERVAL_SECONDS,
          );
        } catch (error) {
          ...
          data.upstream_keyword_capture_switch_ttl_seconds =
            UPSTREAM_KEYWORD_CAPTURE_DEFAULT_SWITCH_TTL_SECONDS;
          data.status_code_retry_enabled = false;
          data.status_code_retry_status_codes =
            STATUS_CODE_RETRY_DEFAULT_STATUS_CODES;
          data.status_code_retry_count = STATUS_CODE_RETRY_DEFAULT_COUNT;
          data.status_code_retry_channel_id =
            STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID;
          data.status_code_retry_interval_seconds =
            STATUS_CODE_RETRY_DEFAULT_INTERVAL_SECONDS;
        }
      } else {
        data.force_format = false;
      ...
      setChannelSettings({
        ...
        upstream_keyword_capture_switch_ttl_seconds:
          data.upstream_keyword_capture_switch_ttl_seconds ||
          UPSTREAM_KEYWORD_CAPTURE_DEFAULT_SWITCH_TTL_SECONDS,
        status_code_retry_enabled: data.status_code_retry_enabled || false,
        status_code_retry_status_codes:
          data.status_code_retry_status_codes ||
          STATUS_CODE_RETRY_DEFAULT_STATUS_CODES,
        status_code_retry_count:
          data.status_code_retry_count || STATUS_CODE_RETRY_DEFAULT_COUNT,
        status_code_retry_channel_id:
          data.status_code_retry_channel_id ??
          STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID,
        status_code_retry_interval_seconds:
          data.status_code_retry_interval_seconds ??
      ...
      const hasAdvancedValues =
        ...
        data.status_code_retry_enabled ||
        (data.status_code_retry_status_codes &&
          data.status_code_retry_status_codes.trim() &&
          data.status_code_retry_status_codes.trim() !==
            STATUS_CODE_RETRY_DEFAULT_STATUS_CODES) ||
        (data.status_code_retry_count ?? STATUS_CODE_RETRY_DEFAULT_COUNT) !==
          STATUS_CODE_RETRY_DEFAULT_COUNT ||
  ...
  const submit = async () => {
    ...
    const channelExtraSettings = {
      ...
      status_code_retry_enabled: localInputs.status_code_retry_enabled === true,
      status_code_retry_status_codes:
        localInputs.status_code_retry_status_codes ||
        STATUS_CODE_RETRY_DEFAULT_STATUS_CODES,
      status_code_retry_count: parsePositiveInteger(
        localInputs.status_code_retry_count,
        STATUS_CODE_RETRY_DEFAULT_COUNT,
      ),
      status_code_retry_channel_id: parseNonNegativeInteger(
        localInputs.status_code_retry_channel_id,
        STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID,
      ),
      status_code_retry_interval_seconds: parseNonNegativeInteger(
        localInputs.status_code_retry_interval_seconds,
        STATUS_CODE_RETRY_DEFAULT_INTERVAL_SECONDS,
      ),
    };
    ...
    delete localInputs.status_code_retry_enabled;
    delete localInputs.status_code_retry_status_codes;
    delete localInputs.status_code_retry_count;
    delete localInputs.status_code_retry_channel_id;
    delete localInputs.status_code_retry_interval_seconds;
                ...
                <div className='py-3 border-b border-gray-100'>
                  ...
                  <div className='mt-3 rounded-lg border border-gray-100 p-3'>
                    ...
                    <div className='mt-3 border-t border-gray-100 pt-3'>
                      ...
                      <Text type='tertiary' size='small'>
                        {t(
                          '默认：目标渠道 15，切换 1 次，有效期 600 秒（10 分钟）',
                        )}
                      </Text>
                    </div>
                  </div>
                  <div className='mt-3 rounded-lg border border-gray-100 p-3'>
                    <Form.Switch
                      field='status_code_retry_enabled'
                      label={t('状态码重试')}
                      checkedText={t('开')}
                      uncheckedText={t('关')}
                      onChange={(value) =>
                      ...
                      extraText={t(
                        '命中上游状态码后，在当前请求内重发原请求体',
                      )}
                    />
                    <Row gutter={12}>
                      <Col span={6}>
                        <Form.Input
                          field='status_code_retry_status_codes'
                          label={t('状态码')}
                          placeholder={STATUS_CODE_RETRY_DEFAULT_STATUS_CODES}
                          showClear
                          onChange={(value) =>
                            handleChannelSettingsChange(
                              'status_code_retry_status_codes',
                      ...
                      <Col span={6}>
                        <Form.InputNumber
                          field='status_code_retry_count'
                          label={t('重试次数')}
                          min={1}
                          placeholder={String(STATUS_CODE_RETRY_DEFAULT_COUNT)}
                          onNumberChange={(value) =>
                            handleChannelSettingsChange(
                              'status_code_retry_count',
                              value,
                      ...
                      <Col span={6}>
                        <Form.InputNumber
                          field='status_code_retry_channel_id'
                          label={t('重试渠道 ID')}
                          min={0}
                          placeholder={String(
                            STATUS_CODE_RETRY_DEFAULT_CHANNEL_ID,
                      ...
                      <Col span={6}>
                        <Form.InputNumber
                          field='status_code_retry_interval_seconds'
                          label={t('重试间隔秒')}
                          min={0}
                          placeholder={String(
                            STATUS_CODE_RETRY_DEFAULT_INTERVAL_SECONDS,
                    ...
                    <Text type='tertiary' size='small'>
                      {t(
                        '状态码支持逗号和范围，例如 429,500-503；重试渠道 ID 为 0 表示原渠道，大于 0 表示指定渠道。默认：429，重试 1 次，渠道 0，间隔 0 秒',
                      )}
                    </Text>
                  </div>
                  <Form.TextArea
